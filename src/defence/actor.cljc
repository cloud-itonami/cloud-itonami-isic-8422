(ns defence.actor
  "DefenceProcurementActor — the ISIC Rev.5 8422 defence procurement and
  logistics administration actor as a `langgraph.graph/state-graph`
  (per ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  procurement or logistics operation request (intake → advise → govern →
  ...
  The unconditional invariant: the DefenceProcurementAdvisor can never
  directly commit a record or dispatch a robot action the
  DefenceProcurementGovernor refuses — every commit-record! call is gated
  behind `:decide`. Additionally, any proposal touching weapons,
  personnel deployment, classified operations, or lethal decisions
  PERMANENTLY HOLDS (no path to approval)."
...
(defn build-graph
  "Build a compiled DefenceProcurementActor graph. `store` implements
  `defence.store/Store`. `advisor` implements `defence.advisor/Advisor`
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:vendor-id (:vendor-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
...
(defn run-request!
  "Run one procurement/logistics operation request to completion or
